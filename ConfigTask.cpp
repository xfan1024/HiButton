...
static uint16_t constexpr config_udp_port = 7866;
...
static inline ConfigImpl* as_config_impl(TaskImpl *impl)
{
    return static_cast<ConfigImpl*>(impl);
}

ConfigTask::ConfigTask() : Task(new ConfigImpl)
{
}

ConfigTask::~ConfigTask()
{
    delete as_config_impl(m_impl);
}

void ConfigTask::on_config_wifi(const std::function<void(const char *ssid, const char *key)> &cb)
{
    as_config_impl(m_impl)->on_config_wifi(cb);
}
